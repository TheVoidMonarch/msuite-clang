...
void displayGraphicalPrayerTimesLocal(PrayerTimes pt) {
    ...
    // Fallback font for punctuation
    TTF_Font* fallbackFont = TTF_OpenFont("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 28);

    if (!nextFont || !listFont || !fallbackFont) {
    ...
    while (!quit) {
        ...
        if (next_prayer_time_sec != 0) {
            ...
            // Render "Next Prayer" line with mixed fonts
            int xPos = 170; // shifted 30px left
            int yPos = 75;

            render_text(renderer, nextFont, "Next Prayer", xPos, yPos, textColor);
            render_text(renderer, fallbackFont, ":", xPos + 200, yPos, textColor);
            render_text(renderer, nextFont, next_prayer_name, xPos + 220, yPos, textColor);

            snprintf(buffer, sizeof(buffer), "( %02d:%02d:%02d left )", hours, minutes, seconds);
            render_text(renderer, fallbackFont, buffer, xPos + 420, yPos, textColor);
        }

        // Prayer list shifted 7px left
        int startX = 253;
        int startY = 170;
        int spacing = 40;

        for (int i = 0; i < 6; i++) {
            render_text(renderer, listFont, labels[i], startX, startY + i * spacing, textColor);
            render_text(renderer, fallbackFont, ":", startX + 120, startY + i * spacing, textColor);
            render_text(renderer, listFont, values[i], startX + 140, startY + i * spacing, textColor);
        }
